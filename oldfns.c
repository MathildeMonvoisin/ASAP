...
void myclose(Parameter asap_param)
{
	if (asap_param.web == 1)
	{
		fclose(asap_param.fres);
		exit_properly(asap_param.ledir);
	}
	else
		exit(1);
}
...
/*--------------------------------------------*/
/*check that names are uniques in alignment*/
/* returns 0 if the name is the same, 1 otherwise */
...
short compare_DNA(char s1, char s2)
{
	...
	*/
	...
	if (s1 == '-' || s2 == '-' || s1 == '+' || s2 == '+')
	{ /* consider deletion as a diff only if the alignemt has been recoded */
	...
	*/
	if (s1 == 'A')
	{
		if (s2 == 'M' || s2 == 'R' || s2 == 'W' || s2 == 'V' || s2 == 'H' || s2 == 'D')
			return 1;
		else
			return 0;
	}
	if (s2 == 'A')
	{
		if (s1 == 'M' || s1 == 'R' || s1 == 'W' || s1 == 'V' || s1 == 'H' || s1 == 'D')
			return 1;
		else
			return 0;
	}
	...
	*/
	if (s1 == 'C')
	{
		if (s2 == 'M' || s2 == 'S' || s2 == 'Y' || s2 == 'V' || s2 == 'H' || s2 == 'B')
			return 1;
		else
			return 0;
	}
	if (s2 == 'C')
	{
		if (s1 == 'M' || s1 == 'S' || s1 == 'Y' || s1 == 'V' || s1 == 'H' || s1 == 'B')
			return 1;
		else
			return 0;
	}
	...
	*/
	if (s1 == 'G')
	{
		if (s2 == 'R' || s2 == 'S' || s2 == 'K' || s2 == 'V' || s2 == 'D' || s2 == 'B')
			return 1;
		else
			return 0;
	}
	if (s2 == 'G')
	{
		if (s1 == 'R' || s1 == 'S' || s1 == 'K' || s1 == 'V' || s1 == 'D' || s1 == 'B')
			return 1;
		else
			return 0;
	}
	...
	*/
	if (s1 == 'T')
	{
		if (s2 == 'W' || s2 == 'Y' || s2 == 'K' || s2 == 'H' || s2 == 'D' || s2 == 'B')
			return 1;
		else
			return 0;
	}
	if (s2 == 'T')
	{
		if (s1 == 'W' || s1 == 'Y' || s1 == 'K' || s1 == 'H' || s1 == 'D' || s1 == 'B')
			return 1;
		else
			return 0;
	}
...
int ReadFastaSequence(FILE *f, struct FastaSeq *laseq, int *lenseq)
{
	...
	while (1)
	{
		...
		if (c == '>')
		{
			ungetc(c, f);
			break;
		} // put back in the stream the next new seq indicator
		// if (c != '\n' && c != '\r' && c != '\t' && c != ' ')
		if (c != '\n' && c != '\r' && c != '\t' && c != ' ' && c != '?')
		{
...
char **getMultPartData(int *nb, char *nf, int *no_err)
{
	...
	for (i = 0;;)
	{
		...
		k = 0;
		while (*(newName + k) != '"' && (newName + k) != NULL)
			k++; // look for end of name
		if ((newName + k) == NULL)
			// fprintf(fres, "PB parsing form <BR>\n"), fclose (fres), exit_properly(ledir);
			if (k == 0)
			{
				*no_err = 7;
				return (NULL);
			}; // should never arrives exept if someone put a field with no name in the html form...
		...
		if (strstr(cgivars[i], "ubmit") != NULL)
		{
			paircount = i;
			break;
		} // last field is button submit and we don't care
...
struct DistanceMatrix compute_dis(FILE *f, int method, float ts_tv, int *len_seq, Parameter asap_parameter)
{
	...
	while (i)
	{
		...
		if (nseq == nalloc)
		{
			...
			if (mesSeq == NULL)
			{
				fprintf(fres, "not enough memory\n");
				exit(1);
			}
		}
	}
...
// double distanceJC69 (char *s1,char *s2, int l)
{
	...
	for (a = 0; a < nseq - 1; a++)
	{
		...
		for (b = a + 1; b < nseq; b++)
		{
			...
			if (ggg < COMMON_SYMBOL)
			{
				// fprintf(fres, "<BR><BR><BR><H3>Sequence %s and %s have %d common site. Distance can't be computed. Bye <BR>", mymat.names[a], mymat.names[b], ggg);
				fprintf(fres, "<BR><BR><BR><H3>Sequence %s and %s have %d common site. Distance is considered infinite.", mymat.names[a], mymat.names[b], ggg);
				// replace by -1/inf?
				// v = -1;
...
struct DistanceMatrix GetDistMat(int nseq, struct FastaSeq *mesSeqs, int method, float ts_tv, Parameter asap_param)
{
	...
	if (!my_mat.names)
		fprintf(fres, "read_distmat: cannot allocate my_mat.names, bye<BR>"), myclose(asap_param);
	...
	if (!my_mat.dist)
		fprintf(fres, "read_distmat: cannot allocate my_mat.dist, bye<BR>"), myclose(asap_param);
	for (a = 0; a < my_mat.n; a++)
	{
	...
	for (a = 0; a < my_mat->n; a++)
	{
		...
		for (b = 0; b < a; b++)
		{
			...
			else
			{
				...
				{
					if (*pt == 10 || *pt == 13)
						fprintf(fres, "readMatrixMegaCVS 10 nbr of seqs %d  not Ok for %s<BR>\n", b, my_mat->names[a]), fclose(fres), exit_properly(ledir);

					int kkk = sscanf(pt, "%f", &d);
					if (kkk != 1)
					{
						fprintf(fres, "read matrix CVS 10 not well formated %d read %d %d (%ld)--%f\n", kkk, a, b, my_mat->n, d);
						fclose(fres);
						exit_properly(ledir);
					}
				}
...
void readMatrixMegaCVS_string(char *data, struct DistanceMatrix *my_mat, char *ledir, FILE *fres)
{
	...
	if (*pt == '\0')
	{
		fprintf(fres, "**read matrix CVS not well formated**\n");
		fclose(fres);
		exit_properly(ledir);
	}
	...
	if (*pt == ',')
	{
		...
		readMatrixMega10CVS_string(pt, my_mat, ledir, fres);
		return;
	}
	...
	for (a = 0; a < my_mat->n; a++)
	{
		if (*pt2 == ',' || *pt2 == 10 || *pt2 == 13 || *pt2 == ' ')
			pt2++; // skip any no need chars
		ll = strchr(pt2, ',');
		if (ll != NULL)
		{
			n = strchr(pt2, ',') - pt2;
		...
		for (b = 0; b <= a; b++)
		{
			if (*pt2 == ',')
			{
				my_mat->dist[a][b] = my_mat->dist[b][a] = 0;
				pt2++;
			}
			else
			{
...
struct DistanceMatrix read_distmat_string(char *data, int fmega, char *ledir, FILE *fres)
{
	...
	else /* classic phylip distance matrix*/
	{
		...
		while (!isdigit(*data) && *data != '\0')
			data++; // skip any  trailing spaces
		if (*data == '\0')
			f_html_error(10, ledir, fres);
		...
		*/
		my_mat.names = (char **)malloc((size_t)sizeof(char *) * my_mat.n + 1);
		if (!my_mat.names)
			fprintf(fres, "read_distmat_string:  cannot allocate my_mat.names, bye<BR>"), fclose(fres), exit_properly(ledir);
		/*		for(a=0;a<my_mat.n; a++){
					my_mat.names[a] = (char *)malloc( (size_t) sizeof(char)*SIZE_NAME_DIST+1 );
					if( ! my_mat.names[a] )
		...
		*/
		my_mat.dist = (double **)malloc((size_t)sizeof(double *) * my_mat.n + 1);
		if (!my_mat.dist)
			fprintf(fres, "read_distmat_string: cannot allocate my_mat.dist, bye<BR>"), fclose(fres), exit_properly(ledir);
		for (a = 0; a < my_mat.n; a++)
		{
			my_mat.dist[a] = (double *)malloc((size_t)sizeof(double) * my_mat.n + 1);
			if (!my_mat.dist[a])
		...
		while (1)
		{
			...
			if (nbc == my_mat.n) // we have found a new name so store it
			{
				...
				if (my_mat.names[nbl] == NULL)
					fprintf(fres, "Not enough memory for %d names<BR>", nbl), fclose(fres), exit_properly(ledir);
			...
			else
			{
				...
				if (strlen(end) > 0)
					fprintf(fres, " Pb in your matrix file only symetrical matrix are supported %d %d (%f) %s %s<BR>", nbl, nbc, tempo, ptr, end), fclose(fres), exit_properly(ledir);
				nbc++;
			}
		}
...
struct DistanceMatrix read_fasta_and_compute_dis(char *input, int method, float ts_tv, Parameter *asap_param)
{
	...
	while ((*input != '>') && *input != '\0')
		input++; // skip any  trailing spaces
	...
	{
		if (*input == '>')
		{ /* if any, the previous seq ends */
			if (nseq == 0)
				length = n;
			if (nseq != -1) // not the fisrt seq so sizeseq is ok
			{
				if (n != length)
					fprintf(fres, "READFASTA:All sequences must be same size (check sequence %d)\n", nseq + 1), fclose(fres), exit_properly(ledir);
				; // at least one seq with different length is found
				myseq[n] = '\0';
				mesSeq[nseq].seq = malloc(sizeof(char) * length + 1);
				if (!mesSeq[nseq].seq)
					fprintf(fres, "READFASTA:MEMORY ERROR error can allocate mesSeq\n"), fclose(fres), exit_properly(ledir);
				;
			...
			if (nseq >= 128 * redo) // need to add some more memory
			{
				...
				if (!mesSeq)
					fprintf(fres, "READFASTA:MEMORY ERROR error can reallocate mesSeq\n"), fclose(fres), exit_properly(ledir);
				;
			}
			k = 0;
			input++; // skip the trailing ">"
			//	fprintf(fres,">> %.10s\n<BR>",input);
			while (*(input + k) != 10 && *(input + k) != 13 && *(input + k) != 0)
				k++;
			if (*(input + k) == 0)
				fprintf(fres, "READFASTA:Pb with File, Please check your data\n"), fclose(fres), exit_properly(ledir);
			;
			/*			    if (k>SIZE_NAME_DIST)
								nk=SIZE_NAME_DIST;
							else*/
			...
			if (!mesSeq[nseq].name)
				fprintf(fres, "READFASTA:MEMORY ERROR error can allocate mesSeq names\n"), fclose(fres), exit_properly(ledir);
		...
		else if (*input != 10 && *input != 13 && *input != 32 && *input != 9) // store the sequence
		{
			...
			if (strchr(nucs, toupper(*input)) == NULL)
				fprintf(fres, "READFASTA:Some unexpected symbol was found:(%c)\n", *input), fclose(fres), exit_properly(ledir); /*weird symbol found*/
			...
			if (nseq == 0)		  /*only for the 1st seq*/
				if (n % 100 == 0) /*give another 100 extras char if not enough room for seqs*/
				{
					...
					if (!myseq)
						fprintf(fres, "READFASTA:MEMORY ERROR cant reallocate myseq\n"), fclose(fres), exit_properly(ledir);
				}
		}
		// fprintf(fres,"----> %.10s\n<BR>",input);
		input++;
	...
	mesSeq[nseq].seq = malloc(sizeof(char) * length + 1);
	if (!mesSeq[nseq].seq)
		fprintf(fres, "READFASTA:MEMORY ERROR error can allocate mesSeq[%d].seq\n", nseq), fclose(fres), exit_properly(ledir);
	//*ll = length;
...
struct DistanceMatrix read_distmat(FILE *f_in, float ts_tv, char *ledir, FILE *fres)
{
	...
	*/
	...
	if (!my_mat.names)
		fprintf(fres, "read_distmat:cannot allocate my_mat.names, bye\n"), fclose(fres), exit_properly(ledir);
	...
	my_mat.dist = (double **)malloc((size_t)sizeof(double *) * my_mat.n);
	if (!my_mat.dist)
		fprintf(fres, "read_distmat:cannot allocate my_mat.dist, bye"), fclose(fres), exit_properly(ledir);
	for (a = 0; a < my_mat.n; a++)
	{
		my_mat.dist[a] = (double *)malloc((size_t)sizeof(double) * my_mat.n);
		if (!my_mat.dist[a])
	...
	for (a = 0; a < my_mat.n; a++)
	{
		...
		*/
		...
		if (feof(f_in))
			fprintf(fres, "read_distmat: PB in dist file format\n"), fclose(fres), exit_properly(ledir);

...
/*replaces strcasestr which doesn't exist on Windows*/
...
void read_mega10(FILE *f_in, struct DistanceMatrix *my_mat)
{
	...
	*ligne = '\0';
	...
	if (!my_mat->names)
		fprintf(stderr, "read_distmat: cannot allocate my_mat.names, bye"), exit(4);
	...
	*/
	...
	if (!my_mat->dist)
		fprintf(stderr, "read_distmat: cannot allocate my_mat.dist, bye"), exit(4);
	for (a = 0; a < my_mat->n; a++)
	{
	...
	for (a = 0; a < my_mat->n; a++)
	{
		...
		for (b = 0; b < a; b++)
		{
			...
			if (feof(f_in))
				printf("your MEGA format can't be read. asap only reads MEGA 6 or MEGA X csv format\n"), exit(1);
			nombre[c] = '\0';
			//
			...
			else
			{
				...
				if (my_mat->dist[b][a] < 0)
				{
					if (w == 0)
					{
						printf("negative distances have been found. Asap can't deal with that, your distance will be assumed to be 0\n");
						w = 1;
					}
					my_mat->dist[b][a] = my_mat->dist[a][b] = 0;
				}
...
void readMatrixMegaCVS(FILE *f_in, struct DistanceMatrix *my_mat)
{
	...
	*ligne = '\0';
	while (1)
	{
		ligne = my_get_line(ligne, f_in, &nbcharmax);
		// printf("%d ->%s\n",nb,ligne);
		if (strncmp(ligne, "Table", 5) == 0 || feof(f_in))
			break;
		if (strlen(ligne) > 2)
		{
			nb++;
...
void readMatrixMega(FILE *f_in, struct DistanceMatrix *my_mat)
{
	...
	while (1)
	{
		...
		if (strcasestr(ligne, "DataFormat=") != NULL)
		{
			if (strcasestr(ligne, "Lowerleft") != NULL)
				lower = 1;
			else if (strcasestr(ligne, "upperright") != NULL)
				lower = 0;
			else
	...
	if (my_mat->n == 0)
		printf("asap was not able to read your MEGA file: [TAXA] number not in the header\n"), exit(1);
	...
	for (a = 0; a < my_mat->n; a++)
	{
		...
		if (feof(f_in))
			printf("problem reading your file\n"), exit(1);
...
int main(int argc, char **argv)
{
	...
	*/
	while ((c = getopt(argc, argv, "o:l:n:p:w:d:t:amuhr:b:x:")) != -1)
	{
		...
		case 'o': /*dir where results files are written*/
			...
			{
				// mkdir(dirfiles, 0700); // error -> one argument
				mkdir(dirfiles);
			}
	...
	{
		simple_name = malloc(sizeof(char) * (strlen(file_data) + 1));
		sprintf(simple_name, "%s", file_data);
	...
	if (dirfiles == NULL)
	{
		...
		if (!dirfiles)
			fprintf(stderr, "main: cannot allocate dirfiles bye\n"), exit(2);
		dirfiles[0] = '.';
		dirfiles[1] = '/';
		dirfiles[2] = '\0';
	}
	...
	*/
	...
	if (f_in == NULL)
		fprintf(stderr, "cannot open the file_data, bye\n"), exit(1);
	...
	if (asap_param.onlyspart == 0)
	{
		...
		if (file_res_cvs == NULL)
			fprintf(stderr, "cannot open the result  output file %s, bye\n", name_res_cvs), exit(1);
		// changed
	}
	...
	if (asap_param.onlyspart == 0)
	{
		...
		if (svgout == NULL)
			fprintf(stderr, "cannot open the graphic output file %s, bye\n", fname), exit(1);
	}
	...
	*/
	...
	if (!ListDistance)
		fprintf(stderr, "main: cannot allocate  ListDistance bye\n"), exit(2);
	//
	...
	if (!no_node)
		fprintf(stderr, "main: MEMORY ERROR error can allocate nonode bye\n"), exit(2);

	zenodes = (Node *)malloc((size_t)sizeof(Node) * ((mat.n * 2) - 1));
	if (!zenodes)
		fprintf(stderr, "main: MEMORY ERROR error can allocate  zenodes bye\n"), exit(2);

	strucompo = (Tabcompo *)malloc((size_t)sizeof(Tabcompo) * mat.n);
	if (!strucompo)
		fprintf(stderr, "main: cannot allocate  strucompo bye\n"), exit(2);
	...
	if (!scores)
		fprintf(stderr, "main: cannot allocate  scores bye\n"), exit(2);
